...
subjID = 'J08'; % Name of the monkey
date = 20210225; % Date of recording
...
arrayToStim = ROIs{1}; % CHANGE THIS LINE TO CHOOSE WHICH ARRAY TO STIMULATE
...
% Trial params
params.nTrialsPerWaveform = 10; % repeats per combination
params.minPause = 19.9999; %seconds
params.maxPause = 20; %seconds

% Pulse params
params.amplitude = [15 100]; % microAmps
params.frequency = [250]; % Hz

%% Choose electrodes to stimulate

stimElecs = [8 7 6 5 4 3 2 1]; % Choose which electrodes to stimulate
...
for iStim = 1:size(shuffled,2)
    ...
    stimulator.autoStim(stimChans(1:2:end-1), 1);%autoStim(Channel,waveformID)
    stimulator.autoStim(stimChans(2:2:end), 2);%autoStim(Channel,waveformID)
    stimulator.endGroup();
...
cd(directories.recording)
save('trialInfo_FlankProp2.mat','trialInfo','stimChans','stimElecs');
save('waveformParsIds_FlankProp2.mat','waveTypes');
