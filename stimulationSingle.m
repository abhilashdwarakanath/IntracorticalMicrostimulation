...
% Trial params
params.nTrialsPerWaveform = 50; % repeats per combination
params.minPause = 10; %seconds
params.maxPause = 10.3; %seconds

% Pulse params
params.amplitude = [15]; % microAmps
params.frequency = [250]; % Hz

%% Choose electrodes to stimulate

stimElecs = [53 54]'; % Choose which electrodes to stimulate
...
for iStim = 1:size(shuffled,2)
    ...
    stimulator.autoStim(stimChans(1), 1);%autoStim(Channel,waveformID)
    stimulator.autoStim(stimChans(2), 2);%autoStim(Channel,waveformID)
    stimulator.endGroup();
...
cd(directories.recording)
save('trialInfo_SingleProp2.mat','trialInfo','stimChans','stimElecs');
save('waveformParsIds_SingleProp2.mat','waveTypes');
